...
#for computecanada
    """
      echo "--------tensorflow---------" &> {log}
      pip install tensorflow_cpu==1.13.1+computecanada --no-index >>& {log}
      echo "--------other dependencies---------" >>& {log}
      pip install \
        requests \
        PyYAML \
        connexion==1.4.2 \
        ...
        jsonschema==2.6.0 >>& {log}

      echo "--------pvactools--------" >>& {log}
      pip install pvactools[API] --no-deps >>& {log}
      echo "--------vcf-annotation-tools--------" >>& {log}
      pip install vcf-annotation-tools >>& {log}
      echo "--------install_vep_plugin--------" >>& {log}
      pvacseq install_vep_plugin pVACtools/make_env >>& {log}
      echo "--------all done--------" >>& {log}
      mhcflurry-downloads fetch >>& {log}
