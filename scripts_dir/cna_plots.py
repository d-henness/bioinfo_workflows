    ...
    with open(filenm, 'r') as data:
        for i, line in enumerate(data):
            if i != 0:
                ...
                if split_line[5] != 'NA':
                    needed_data[filenm].append([split_line[0], int(split_line[1]) + chrom_offset[split_line[0]], float(split_line[5]), split_line[4]])
...
axes = fig.subplots(len(args.filenms), 1, sharex = 'all', sharey = 'all')
# axes needs to be a list later in the code so make it one if it isn't already
if len(args.filenms) == 1:
    axes = [axes]

colour_map = {'NEUT': 'blue', 'GAIN': 'brown', 'HETD': 'green', 'AMP': 'red', 'HLAMP': 'red'}

for filenm in args.filenms:
    for i, data in enumerate(needed_data[filenm]):
        if data[3] not in colour_map:
            sys.exit(f'Erorr: missing {data[3]} in {filenm} from colour_map')
        else:
            needed_data[filenm][i][3] = colour_map[data[3]]
...
for i, filenm in enumerate(args.filenms):
    ...
    y = [data[2] for data in needed_data[filenm]]
    colours = [data[3] for data in needed_data[filenm]]
    axes[i].scatter(x, y, c = colours, s = 2)
    axes[i].set_ylabel(os.path.split(filenm)[1].split('.')[0])
    axes[i].vlines([chrom_offset[chrom] for chrom in chroms], ymin = min_val - 0.1, ymax = max_val + 0.1)
    axes[i].set_xlim(0, chrom_offset['chrX'] + chr_lengths['chrX'])
    axes[i].set_ylim(bottom = min_val - 0.1, top = max_val + 0.1)
    if i == len(args.filenms) - 1:
        axes[i].set_xticks([tick[1] for tick in xtick_labels])
        axes[i].set_xticklabels(tick[0] for tick in xtick_labels)
