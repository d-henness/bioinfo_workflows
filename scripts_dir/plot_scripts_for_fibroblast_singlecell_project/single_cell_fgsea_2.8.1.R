...
print(paste0("Will runs on these types: ", unique(just_subset_diag$new_cat_labels)))

for (cell_type in unique(just_subset_diag$new_cat_labels)){
    print(paste0("Will run on: ", cell_type))
    outdir <- file.path(args$outdir, args$diagnosis, cell_type, "DE")
    markers <- run_DE(just_subset_diag, cell_type, "new_cat_labels", outdir, args$min_cells)
    if (!is.null(markers)){
