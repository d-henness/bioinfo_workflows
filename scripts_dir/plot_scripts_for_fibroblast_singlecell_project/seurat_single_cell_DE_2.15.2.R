...
make_gene_set_plots <- function(markers, outdir, min_val = -2, max_val = 2){
    ...
    if (length(all_genes) > 0) {
        annotation_row <- data.frame(
            gene_set = factor(sapply(all_genes, function(g) names(which(sapply(gene_sets, function(gs) g %in% gs)))[1])),
            row.names = all_genes
        )
        all_plot <- pheatmap(
            markers[all_genes, , drop = FALSE],
            cluster_rows    = TRUE,
            cluster_cols    = TRUE,
            scale           = "none",
            main            = "avg_log2FC (all gene sets)",
            fontsize_row    = 8,
            fontsize_col    = 8,
            color           = color_palette,
            breaks          = breaks,
        ...
        filenm <- file.path(outdir, "heatmap_diff_express_all_gene_sets.pdf")
        height <- max(5, length(all_genes) / 5)
        width  <- length(colnames(markers)) / 5
        ggsave(filenm, all_plot, width = width, height = height, limitsize = FALSE)
    }
}
...
run_DE <- function(seurat_object, cell_type, identity_of_interest, outdir, file_suff, min_cells){
    ...
    if (length(good_samples) == 0) {
        print(paste0("Skipping ", cell_type, ": no samples passed the 10-cell filter"))
        return(NULL)
    }
...
all_markers <- list()
for (m_label in unique(just_subset$m_labels)){
    just_subset_new <- subset(just_subset, m_labels == m_label)
    just_subset_new$m_labels <- sanitize_label_for_AggregateExpression(just_subset_new$m_labels)

    for (diag in unique(just_subset_new$diagnosis)){
        if (diag != "normal_skin") {
            diag_and_m = paste0(diag, "_", m_label)
            all_markers[[diag_and_m]] <- run_DE(just_subset_new, diag, "diagnosis", file.path(args$outdir, m_label), "vs_normal_skin", args$min_cells)
        }
    }
}
...
for (diag_and_m in names(all_markers)) {
    diag    <- Filter(function(d) startsWith(diag_and_m, d), unique(just_subset$diagnosis))[[1]]
    m <- sub(paste0("^", diag, "_"), "", diag_and_m)
    new_df <- data.frame(
        genes     = avail_genes,
        diag      = diag,
        m_label   = m,
        log2FC    = all_markers[[diag_and_m]][avail_genes, "avg_log2FC"],
        p_val     = all_markers[[diag_and_m]][avail_genes, "p_val"],
        p_val_adj = all_markers[[diag_and_m]][avail_genes, "p_val_adj"]
    )
...
for (gene_set in names(gene_sets)){
    ...
    for (diag_and_m in names(all_markers)) {
        ...
        new_df  <- data.frame(
            ...
            diag      = diag,
            m_label   = m,
            log2FC    = all_markers[[diag_and_m]][gene_set_genes, "avg_log2FC"],
            p_val     = all_markers[[diag_and_m]][gene_set_genes, "p_val"],
            p_val_adj = all_markers[[diag_and_m]][gene_set_genes, "p_val_adj"]
        )
        marker_matrix <- rbind(marker_matrix, new_df)
    }
...
for (diag_and_m in names(all_markers)) {
    marker_matrix[[diag_and_m]]  <- all_markers[[diag_and_m]][avail_genes, "avg_log2FC"]   
}
