...
#        geom_vline(xintercept = line_positions, color = "black", linewidth = 0.3)
    ...
    width <- 4 * length(gene_set) / 10
    height <- max(6, 2 * (nrow(unique(seurat_object[[identity]])) / 10))
    print(paste0("width: ", width, " height: ", height))

    ggsave(paste0(filenm, ".pdf"), plot, width = width, height = height, limitsize = FALSE)
...
for (diag in unique(just_subset$diagnosis)){
    ...
    for (gs_name in names(all_gene_sets)){
        ...
        make_plot(just_diag, gene_set, NULL, "m_labels", filenm)
    }
}
