...
run_DE <- function(seurat_object, cell_type, identity_of_interest, outdir, file_suff, min_cells){
    ...
    bulk <- AggregateExpression(
        ...
        slot = "counts",
        assays = "RNA",
        group.by = c("singleR.labels_fine", "sample_id")
    )
...
for (label_fine in unique(just_subset$singleR.labels_fine)){
    run_DE(just_subset_diag, label_fine, "singleR.labels_fine", file.path(args$outdir, args$cell_type), "vs_all", args$min_cells)
}
